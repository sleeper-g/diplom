<x-cinema-layout>
    @php
        ...
        for ($i = 0; $i < 7; $i++) {
            ...
            $days[] = [
                'date' => $date,
                'dayOfWeek' => $date->locale('ru')->dayName,
                'dayNumber' => $date->day,
                'isToday' => $i === 0,
    ...
    <nav class="page-nav">
        @foreach($days as $day)
            <a class="page-nav__day {{ $day['isToday'] ? 'page-nav__day_today' : '' }} {{ $day['isWeekend'] ? 'page-nav__day_weekend' : '' }} {{ request('date') == $day['date']->format('Y-m-d') ? 'page-nav__day_chosen' : '' }}" 
               href="{{ route('home', ['date' => $day['date']->format('Y-m-d')]) }}">
